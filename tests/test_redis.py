from src.rate_limiter.decorator import rate_limit_redis
...
@app.get("/test-basic")
@rate_limit_redis("3/minute")
def basic_memory(request: Request):
    return {
        "message": "Success!",
...
@app.get("/test-user")
@rate_limit_redis("5/minute", key_func=get_test_user_id)
def user_route_memory(request: Request):
...
def get_numeric_test_key(request):
    return "numeric_test_key"

@app.get("/test-numeric")
@rate_limit_redis(1, 1, 1, key_func=get_numeric_test_key)
...
def test_token_refill():
    ...
    @app.get("/test-refill")
    @rate_limit_redis(1, 1, 2, key_func=lambda r: "test_refill_key")
